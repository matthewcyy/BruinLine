...
router.post("/addFood", async (req, res) => {
  try {
    let { itemName, onTodaysMenu, diningHall, calories, fat, carbs, protein } =
      req.body;
    if (!itemName || !diningHall || !calories || !fat || !carbs || !protein)
      return res.status(400).json({ msg: "Not all information is filled out" });
    if (!onTodaysMenu) onTodaysMenu = "false";
    const foodItemExists = await Food.findOne({
      itemName: itemName,
...
router.patch("/reviewFood", async (req, res) => {
  try {
    let { itemName, diningHall, rating, date, description, reviewer } =
      req.body;
...
router.get("/getAllReviews", async (req, res) => {
  try {
    ...
    allFoodReviews = allFoodReviews.map(({ username, reviews }) => ({
      username,
      reviews,
    }));
...
router.post("/getNutrientFacts", async (req, res) => {
  try {
    console.log("FOODNAME", req.body.itemName);
    const food = await Food.findOne({ itemName: req.body.itemName });
...
router.patch("/addPatch", async (req, res) => {
