...
router.post("/register", async (req, res) => {
  try {
    ...
    if (password.length < 5)
      // checking if password is too short or not
      return res
        .status(400)
...
router.patch("/inviteToGroup", async (req, res) => {
  ...
  try {
    ...
    if (inviteList.findIndex((x) => x.groupId === req.body.groupId) !== -1)
      return res
        .status(400)
        .json({ message: "User has already been invited to group" }); // when user has already been invited to group
...
router.patch("/acceptInvite", async (req, res) => {
  ...
  try {
    ...
    if (group) {
      ...
      const indexOfInvite = inviteList.findIndex(
...
router.patch("/rejectInvite", async (req, res) => {
  // When using this api call, get req.body information from User and from USER'S INVITE LIST. USED WHEN ACCEPTING INVITATIOn
  try {
    const user = await User.findById(req.body.id); // finding user in database
    if (!user) {
