...
function Reviews() {
  ...
  const [diningHallsWithFoods, setDiningHallsWithFoods] = useState({
    DeNeve: [],
    Feast: [],
    "B-Plate": [],
    Epicuria: [],
  ...
  const getAllReviews = async () => {
    var allReviews = await axios.get(
              ...
              <Grid container xs={12} spacing={0.3}>
                <Grid item xs={4}>
                  <TextField
                    ...
                    variant="outlined"
                    size="small"
                    value={foodItemReview}
                    onChange={(e) => setFoodItemReview(e.target.value)}
                  />
                </Grid>
                <Grid item xs={4}>
                  <TextField
